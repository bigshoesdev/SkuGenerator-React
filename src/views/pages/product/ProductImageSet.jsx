...
class ProductImageset extends React.Component {
    ...
    tableHeads(type) {
        let width;
        if (type == 'toteBags' || type == 'stickers' || type == 'hoodies' || type == 'kids') {
    ...
    dataList(type) {
        ...
        return this.state.data[type].map((data, index) => {
            return (
                <tr key={index}>
                    <th style={{ 'textAlign': 'center', 'color': '#8898aa' }}>{this.state.data_th[index]}</th>
                    <td className="text-center"
                        ...
                        dangerouslySetInnerHTML={{ __html: data.file1 }}
                        style={{ width: '40px', height: '40px', border: this.state.isEdit[type][index] ? '2px solid' : '1px solid rgb(233, 236, 239)' }
                        }>
                    </td>
                    <td className="text-center"
                        contentEditable={this.state.isEdit[type][index]}
                        ...
                        dangerouslySetInnerHTML={{ __html: data.file2 }}
                        style={{ width: '40px', height: '40px', border: this.state.isEdit[type][index] ? '2px solid' : '1px solid rgb(233, 236, 239)' }
                        }>
                    </td>
                    {type == 'toteBags' || type == 'stickers' || type == 'hoodies' || type == 'kids' ||
                        <td className="text-center"
                            contentEditable={this.state.isEdit[type][index]}
                            onInput={e => this.emitChange(e, type, index, 'file3')}
                            onBlur={e => this.emitChange(e, type, index, 'file3')}
                            dangerouslySetInnerHTML={{ __html: data.file3 }}
                            style={{ width: '40px', height: '40px', border: this.state.isEdit[type][index] ? '2px solid' : '1px solid rgb(233, 236, 239)' }
                            }>
                        </td>
                    }
                            ...
                            <Col md={12} xl={12}>
                                <Button className="btn-tbl-tshirtvariant-edit" size="sm" color="primary" data-dz-remove onClick={e => {
                                    ...
                                    <span className="btn-inner--icon mr-1">
                                        <i className={classnames({ 'fas fa-edit': !self.state.isEdit[type][index], 'fas fa-save': self.state.isEdit[type][index] })} />
                                    </span>
                                ...
                                <div className="div-tbl-title-imgVar">
                                    <Row>
                                        <Col style={{ tableLayout: 'fixed', width: "600px" }}>
                                            <h2>
                                                {this.Capitalize(key) + " Image Sets"}
                                            </h2>
                                        </Col>
                                    </Row>
                                </div>
